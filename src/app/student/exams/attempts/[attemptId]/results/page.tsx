...
export default function AttemptResultsPage() {
      ...
      <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-indigo-50">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          <div className="text-center py-16">
            ...
            <h3 className="text-lg font-semibold text-gray-900 mb-2">Chargement des résultats</h3>
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-indigo-50">
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        {/* Score Summary */}
        <div className={`rounded-xl border-2 p-8 mb-8 ${getScoreBg(result.percentage)}`}>
          <div className="text-center">
            <div className="flex justify-center mb-4">
              {getScoreIcon(result.percentage)}
            </div>
            <h2 className={`text-3xl font-bold mb-2 ${getScoreColor(result.percentage)}`}>
              {result.score} / {result.maxScore} points
            </h2>
            <p className={`text-xl font-semibold mb-4 ${getScoreColor(result.percentage)}`}>
              {result.percentage.toFixed(1)}% - {getScoreLabel(result.percentage)}
            </p>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-8">
              <div className="bg-white/50 rounded-lg p-4">
                ...
                <div className="text-sm font-medium text-gray-500">Temps passé</div>
                <div className="text-lg font-bold text-gray-900">{result.timeSpent} min</div>
              </div>
              <div className="bg-white/50 rounded-lg p-4">
                <div className="flex items-center justify-center mb-2">
                  <CheckCircle className="h-5 w-5 text-green-600" />
                </div>
                <div className="text-sm font-medium text-gray-500">Bonnes réponses</div>
              ...
              <div className="bg-white/50 rounded-lg p-4">
                ...
                <div className="text-sm font-medium text-gray-500">Complété le</div>
                <div className="text-lg font-bold text-gray-900">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 mb-8">
          <h3 className="text-lg font-semibold text-gray-900 mb-4">Informations sur l'examen</h3>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {result.module && (
              <div className="flex items-center gap-2">
                <BookOpen className="h-4 w-4 text-gray-500" />
                <span className="text-sm font-medium text-gray-500">Module:</span>
                <span className="text-sm text-gray-900">{result.module.name}</span>
              </div>
            )}
            {result.lesson && (
              <div className="flex items-center gap-2">
                <BookOpen className="h-4 w-4 text-gray-500" />
                <span className="text-sm font-medium text-gray-500">Leçon:</span>
                <span className="text-sm text-gray-900">{result.lesson.title}</span>
              </div>
            )}
            {result.studyYear && (
              <div className="flex items-center gap-2">
                <Calendar className="h-4 w-4 text-gray-500" />
                <span className="text-sm font-medium text-gray-500">Année d'étude:</span>
                <span className="text-sm text-gray-900">{result.studyYear.name}</span>
              </div>
            )}
            <div className="flex items-center gap-2">
              <Clock className="h-4 w-4 text-gray-500" />
              <span className="text-sm font-medium text-gray-500">Durée:</span>
              <span className="text-sm text-gray-900">{result.timeSpent} minutes</span>
            </div>
          </div>
          {result.description && (
                  ...
                  <div className="flex items-start gap-4">
                    ...
                    <div className="flex-1">
                      ...
                      <div className="space-y-3">
                        ...
                        <div>
                          <span className="text-sm font-medium text-gray-500">Bonne(s) réponse(s):</span>
                        ...
                        {question.explanation && (
                          <div className="mt-4 p-4 bg-blue-50 rounded-lg border border-blue-200">
                            <span className="text-sm font-medium text-blue-900">Explication:</span>
