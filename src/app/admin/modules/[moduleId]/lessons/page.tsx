...
function SortableLessonItem({ lesson, onEdit, onDelete }: SortableLessonItemProps) {
  ...
  return (
    <div
      ref={setNodeRef}
      style={style}
      className={`p-6 flex items-center justify-between hover:bg-gray-50 border-b border-gray-200 transition-colors ${
    ...
    >
      <div className="flex items-center space-x-4 flex-1">
        <div
          {...attributes}
          {...listeners}
          className="cursor-grab hover:cursor-grabbing text-gray-400 hover:text-gray-600 p-1 rounded hover:bg-gray-100 transition-colors"
...
export default function ModuleLessonsPage() {
  ...
  return (
    <AdminLayout
      title={`Gestion des leçons - ${module?.name}`}
      subtitle={`${module?.semester.studyYear?.name || ''} • ${module?.semester.name || ''}`}
    >
